...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        #kernel_size 2->3 avg_pooling ->max_pooling
        self.add_module('pool', nn.MaxPool2d(kernel_size=3, stride=2))
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            if i<3:
                block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                    bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate,dilation=1)
                ...
                block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                    bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate, dilation=6)
                self.features.add_module('denseblock%d' % (i + 1), block)
                num_features = num_features + num_layers * growth_rate
                if i != len(block_config) - 1:
                    trans = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
        ...
        # Linear layer
        self.classifier = nn.Linear(num_features, 80)
        torch.nn.init.xavier_uniform(self.classifier.weight)
        if self.classifier.bias:
            torch.nn.init.constant(self.classifier.bias,0)
...
def focal_loss(x,y,gama):
    ...
    weight = torch.pow(1-torch.exp(-loss),gama)
    loss = loss*weight
