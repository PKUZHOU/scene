...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,dilation=5)
        # self.deform2d = ConvOffset2D(2048)
        # self.conv5 = nn.Conv2d(2048,2048,kernel_size=5,stride=1,padding=0)
        # self.avgpool = nn.MaxPool2d(3,stride=2)
        self.avgpool = nn.AvgPool2d(7)
        self.fc = nn.Linear(512 * block.expansion, num_classes)
        torch.nn.init.xavier_normal(self.fc.weight)
        self.lstm1 = nn.LSTM(input_size=16,hidden_size=1,num_layers=2,batch_first=True,bidirectional=False)
    ...
    def forward(self, x):
        ...
        # x = torch.sum(x,2).view(x.size(0),-1)
        # x = self.avgpool(x)
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            if i != len(block_config) - 1:
                ...
                block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                    bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate,dilation=3)
                self.features.add_module('denseblock%d' % (i + 1), block)
                num_features = num_features + num_layers * growth_rate
...
def focal_loss(x,y,gama):
    log_sum_exp = torch.log((torch.sum(torch.exp(x), 1)))
    _, pred = torch.max(x, 1)
    a = pred!=y
    # counter = torch.sum(a)
    loss = -x.gather(1, y.view(-1, 1))+log_sum_exp
    weight = torch.pow(torch.abs(1-torch.exp(-loss)),gama)
    loss = loss*weight
    # print counter.data[0]
    return torch.sum(loss)/x.data.size(0)
